...
<div class="jumbotron">
  <div class="container text-center">
    <h2>اجمالي التطهيرات/ خطة</h2>
    
<?php

    $sel_query= "SELECT SUM(`p`.`pprice`) `pprice` FROM (SELECT (SELECT SUM(`cost`) FROM `sulfa` WHERE `sulfa`.`pro` = `projects`.`id`) `pprice` FROM `projects` WHERE `type` = 1) `p`;";
